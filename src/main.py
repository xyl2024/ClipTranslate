...
class App:
    def __init__(self):
        ...
        try:
            ...
            # 显示设置窗口提示用户设置API密钥
            QTimer.singleShot(1000, lambda: self.show_settings_with_message("请设置API密钥"))
        
        # 初始化翻译线程
        self.translator_thread = TranslatorThread(self.translator)
        self.translator_thread.translation_done.connect(self.ui_translation.set_translation)
        self.translator_thread.translation_error.connect(self.ui_translation.show_error)
        self.translator_thread.translation_progress.connect(self.ui_translation.update_translation_progress)
    ...
    def translate_clipboard(self, target_lang="Chinese"):
        ...
        if not text.strip():
            self.ui_translation.set_translation("剪贴板为空",
    ...
    def setup_tray_icon(self):
        ...
        translate_to_chinese.triggered.connect(lambda: self.translate_clipboard("Chinese"))
        translate_menu.addAction(translate_to_chinese)
        
        translate_to_english = QAction("翻译为英文", self.app)
        translate_to_english.triggered.connect(lambda: self.translate_clipboard("English"))
    ...
    def check_hotkeys(self):
        try:
            ...
            # 检查中文翻译热键
            if keyboard.is_pressed(self.hotkey_to_chinese) and (current_time - self.last_hotkey_time >= self.hotkey_cooldown):
                logger.debug(f"The hotkey {self.hotkey_to_chinese} is pressed, translating to Chinese")
                self.last_hotkey_time = current_time
                self.translate_clipboard("Chinese")
            
            # 检查英文翻译热键
            elif keyboard.is_pressed(self.hotkey_to_english) and (current_time - self.last_hotkey_time >= self.hotkey_cooldown):
                logger.debug(f"The hotkey {self.hotkey_to_english} is pressed, translating to English")
                self.last_hotkey_time = current_time
    ...
    def show_settings_with_message(self, message):
        ...
        if self.settings_window:
            self.tray_icon.showMessage("设置提示", message, QSystemTrayIcon.Information, 5000)
    ...
    def apply_settings(self, new_config):
        try:
            ...
            # 更新热键
            if (old_config.get("hotkey_to_chinese") != new_config.get("hotkey_to_chinese") or
                old_config.get("hotkey_to_english") != new_config.get("hotkey_to_english")):
                # 热键已更改，需要重新设置
                self.hotkey_to_chinese = new_config.get("hotkey_to_chinese", self.hotkey_to_chinese)
                self.hotkey_to_english = new_config.get("hotkey_to_english", self.hotkey_to_english)
