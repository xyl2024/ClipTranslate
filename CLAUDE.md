...
**技术栈**: Python 3.12, PySide6, `keyboard` 库（全局热键）、`pyperclip`（剪贴板）、`requests`（流式 API）、`uv`（包管理器）
...
### 翻译请求流程
1. 检测到全局热键（F2/F4）或点击托盘菜单
2. `App.translate_clipboard()` 捕获剪贴板文本
3. 校验文本（长度检查、中文检测）
4. 若文本过长，弹出确认对话框
5. `App.start_translation()` 启动 `TranslatorThread`
...
**流式翻译**: 所有翻译器必须支持流式。服务器发送事件（SSE）格式：每行 `data: {...}`。回调接收累积文本块。
**全局热键**: 在 `check_hotkeys()` 中使用 `QTimer` 轮询，间隔 100ms。`keyboard.is_pressed()` 检测按键状态。0.5 秒防抖防止重复触发。
